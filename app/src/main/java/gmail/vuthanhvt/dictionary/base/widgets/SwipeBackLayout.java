...
public class SwipeBackLayout extends ViewGroup {
    ...
    private SwipeBackLayout.DragEdge dragEdge = SwipeBackLayout.DragEdge.TOP;
    ...
    private SwipeBackLayout.SwipeBackListener swipeBackListener;

    private float startX = 0;
    public static final float RANGE_SWIPE = 50f;

    public boolean canSwipe() {
        if (startX <= RANGE_SWIPE) {
            return true;
        }
    ...
    @Deprecated
    public void setOnPullToBackListener(SwipeBackLayout.SwipeBackListener listener) {
        swipeBackListener = listener;
    }

    public void setOnSwipeBackListener(SwipeBackLayout.SwipeBackListener listener) {
        swipeBackListener = listener;
    }
    ...
    public SwipeBackLayout(Context context, AttributeSet attrs) {
        ...
        viewDragHelper =
                ViewDragHelper.create(this, 1.0f, new SwipeBackLayout.ViewDragHelperCallBack());
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        return !handled ? super.
                onInterceptTouchEvent(event) : handled;
    }
    ...
    private class ViewDragHelperCallBack extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            ...
            if (dragEdge == SwipeBackLayout.DragEdge.TOP && !canChildScrollUp() && top > 0) {
                final int topBound = getPaddingTop();
                final int bottomBound = verticalDragRange;
                result = Math.min(Math.max(top, topBound), bottomBound);
            } else if (dragEdge == SwipeBackLayout.DragEdge.BOTTOM
                    && !canChildScrollDown()
                    && top < 0) {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            if (dragEdge == SwipeBackLayout.DragEdge.LEFT && !canChildScrollRight() && left > 0) {
                final int leftBound = getPaddingLeft();
                final int rightBound = horizontalDragRange;
                result = Math.min(Math.max(left, leftBound), rightBound);
            } else if (dragEdge == SwipeBackLayout.DragEdge.RIGHT
                    && !canChildScrollLeft()
                    && left < 0) {
        ...
        public void onViewDragStateChanged(int state) {
            ...
            if ((draggingState == ViewDragHelper.STATE_DRAGGING
                ...
                if (draggingOffset == getDragRange()) {
                    scrollChild.setVisibility(View.GONE);
        ...
        switch (dragEdge) {
            ...
            case BOTTOM:
                if (Math.abs(yvel) > Math.abs(xvel) && Math.abs(yvel) > AUTO_FINISHED_SPEED_LIMIT) {
                    return dragEdge == SwipeBackLayout.DragEdge.TOP ? !canChildScrollUp()
                            : !canChildScrollDown();
                }
                break;
            case LEFT:
            case RIGHT:
                if (Math.abs(xvel) > Math.abs(yvel) && Math.abs(xvel) > AUTO_FINISHED_SPEED_LIMIT) {
                    return dragEdge == SwipeBackLayout.DragEdge.LEFT ? !canChildScrollLeft()
                            : !canChildScrollRight();
                }
                break;
        }
